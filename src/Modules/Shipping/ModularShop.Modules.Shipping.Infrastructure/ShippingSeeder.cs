...
/// <summary>
/// Seed data for Shipping (shipment ids use the prefix <c>40000000-…</c>). These mirror the historical
/// orders seeded by Sales (order ids <c>30000000-…</c>) and reference the same kernel customer ids
/// (<c>20000000-…</c>) so the demo data is coherent. In normal operation Shipping learns about an order
/// ONLY through the <c>OrderPlaced</c> event — this seed is a one-off for a populated first run.
/// </summary>
internal static class ShippingSeed
{
    ...
    public static IReadOnlyList<Shipment> Shipments()
    {
        ...
        var s1 = new Shipment(S(1), "SHP-1001", O(1), "ORD-1001", C(1), "Alice Johnson", now.AddDays(-24));
        ...
        var s2 = new Shipment(S(2), "SHP-1002", O(2), "ORD-1002", C(3), "Carla Nguyen", now.AddDays(-18));
        s2.AddItem("27\" 4K Monitor", 2);
        s2.ApplySeedState(ShipmentStatus.Delivered, "FedEx", "FX558877112", now.AddDays(-17), now.AddDays(-15));
        list.Add(s2);

        var s3 = new Shipment(S(3), "SHP-1003", O(3), "ORD-1003", C(5), "Emma Wilson", now.AddDays(-11));
        ...
        var s4 = new Shipment(S(4), "SHP-1004", O(4), "ORD-1004", C(2), "Bob Martinez", now.AddDays(-7));
        ...
        var s5 = new Shipment(S(5), "SHP-1005", O(5), "ORD-1005", C(7), "George Brown", now.AddDays(-3));
        ...
        var s6 = new Shipment(S(6), "SHP-1006", O(6), "ORD-1006", C(9), "Ivan Petrov", now.AddDays(-1));
